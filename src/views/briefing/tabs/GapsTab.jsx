  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {PRIORITY_SECTIONS.map(({ key, label, color, icon: Icon, tag }) => {
        ...
        return (
          <GapSection key={key} label={label} color={color} icon={Icon} tag={tag} gaps={gaps} />
        );
      })}
    </div>
  );
...
function GapRow({ gap, color, isLast }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            ...
            background: gap.candidate_has ? "var(--success)" : color,
            flexShrink: 0,
          }} />
          <span style={{ fontSize: 15, fontWeight: 600 }}>{gap.skill}</span>
          {gap.candidate_has && (
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--success)", letterSpacing: "0.1em" }}>
              ✓ HAVE
            </span>
          )}
        </div>
        <span style={{
          ...
          padding: "2px 6px", border: `1px solid ${effort.color}44`, borderRadius: 3,
          flexShrink: 0,
        }}>
          {effort.label}
        </span>
      ...
      {/* Market frequency bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--fg-muted)", width: 110 }}>
          MARKET FREQ
        </span>
        <div style={{ flex: 1, height: 4, background: "var(--bg-base)", borderRadius: 2 }}>
          <div style={{
            height: "100%",
            width: `${gap.market_frequency * 100}%`,
            background: color,
            borderRadius: 2,
